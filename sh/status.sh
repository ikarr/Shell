...
Download_Server_Status_server(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/ikarr/ServerStatus/archive/master.zip"
...
Download_Server_Status_client(){
	...
	[[ ! -e ${file} ]] && mkdir "${file}"
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/ikarr/ServerStatus/master/clients/client-linux.py"
...
Service_Server_Status_server(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ikarr/Shell/master/service/server_status_server_centos" -O /etc/init.d/status-server; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ikarr/Shell/master/service/server_status_server_debian" -O /etc/init.d/status-server; then
			echo -e "${Error} ServerStatus 服务端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-server
...
Service_Server_Status_client(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ikarr/Shell/master/service/server_status_client_centos" -O /etc/init.d/status-client; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ikarr/Shell/master/service/server_status_client_debian" -O /etc/init.d/status-client; then
			echo -e "${Error} ServerStatus 客户端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-client
